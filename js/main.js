...
function fillProfileInfo() {
  ...
  networks_info +=
    "<a class='social-icon' href='mailto:" +
    profile.about.email +
    "' target='_blank'><i class='far fa-envelope'></a>";
  ...
  for (var icon in skills) {
    skills_info +=
      "<li class='list-inline-item'><i class='" + skills[icon] + "'></i></li>";
  }
...
function fillPortfolio() {
  ...
  for (var p in portfolio) {
    ...
    portfolio_info +=
      "<a class='portfolio-item' target='_blank' href='" + item["url"] + "'>";
...
function fillPortfolioItems() {
  let portfolio_field = $("#portfolio_accordion");
  let content = "";
  let portfolio = profile.portfolio_items;
  Object.keys(portfolio).map((item) => {
    content += "<button class='accordion'>" + item + "</button>";
    content += "<div class='panel'>";
    portfolio[item].map((descItem) => {
      content += "<p>" + descItem + "</p>";
    });
    content += "</div>";
  });
  $(portfolio_field).html(content);
}
